...
Description :
-------------
Objectif : Gérer les superpositions des fichiers rasters de classes macro d'entrées. Puis réaliser une classification non supervisé par algorithme de kmeans
...
from Lib_file import removeFile, deleteDir
    ...
    if not save_results_intermediate:
        for macroclass_id in range(length_mask):
            ...
            if os.path.isdir(repertory_output_tmp_list[macroclass_id]) :
                deleteDir(repertory_output_tmp_list[macroclass_id])
...
def computeKmeans(image_input, mask_sample_input, image_output, micro_samples_image_out, centroids_file_output, label, number_of_classes, macroclass_id, number_of_actives_pixels_threshold, kmeans_param_minimum_training_set_size, kmeans_param_maximum_iterations, length_mask, no_data_value, rand_otb, ram_otb, codage_8b, codage_16b, format_raster='GTiff', save_results_intermediate=False, overwrite=True):
    ...
    if debug >= 4:
        ...
        print(cyan + "computeKmeans() : " + endC + "kmeans_param_minimum_training_set_size : " + str(kmeans_param_minimum_training_set_size) + endC)
