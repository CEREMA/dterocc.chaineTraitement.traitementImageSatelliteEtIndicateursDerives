...
from Lib_raster import updateReferenceProjection, getGeometryImage, getPixelWidthXYImage, computeStatisticsImage
...
# Les parametres de la fonction OTB otbcli_ComputeImagesStatistics a changé à partir de la version 8.0 de l'OTB
...
for info in pythonpath_list :
    if info.find("OTB") > -1:
        ...
        break
print (otb_info)
if int(otb_info.split(".")[0]) >= 8 :
    IS_VERSION_UPPER_OTB_8_0 = True
else :
    IS_VERSION_UPPER_OTB_8_0 = False
...
def classifySupervised(image_input_list, vector_input, sample_points_values_input, sample_emply_input, classification_file_output, confidence_file_output, model_output, model_input, field_class, sampler_mode, periodic_value, classifier_mode, kernel, rf_parametres_struct, no_data_value, path_time_log, ram_otb=0,  format_raster='GTiff', extension_vector=".shp", save_results_intermediate=False, overwrite=True):
    ...
    if check and not overwrite:
        ...
        if nb_input_images == 1:
            ...
            if IS_VERSION_UPPER_OTB_8_0 :
                command = "otbcli_ComputeImagesStatistics -il %s -out.xml %s" %(list_image_input_str, statistics_image_output)
            else :
