...
""" Notes diverses :
Pour utiliser certains modules (notamment createDatabase() / dropDatabase() / importVectorByOgr2ogr() / exportVectorByOgr2ogr() / importShape() / exportShape() / importRaster()), il est préférable d'être déconnecté de la base de données (conflit d'accès).
...
########################################################################
# Rôle : teste l'existence de la ligne contenant leidentifiants de connexions dans fichier 'pgpass' (fonction qui utilise psql : importShape et importRaster)
# Paramètres en entrée :
#   user_name : nom d'utilisateur du serveur PostgreSQL (par défaut : 'postgres')
#   password : mot de passe du serveur PostgreSQL (par défaut : 'postgres')
#   ip_host : adresse IP du serveur PostgreSQL (par défaut : 'localhost')
