...
from Lib_vector import createPolygonsFromCoordList, geometries2multigeometries, fusionNeighbourPolygonsBySameValue, dissolveVector, getGeomPolygons, createPolygonsFromGeometryList, cleanMiniAreaPolygons, simplifyVector, bufferVector, cleanRingVector
...
#   paramètres :
#       input_dir : dossier contenant les différents dossiers d'images (contenant eux-mêmes les imagettes)
#       output_file : fichier vecteur de sortie contenant les polygones des emprises des images
...
def createEmprise(input_dir, output_file, is_not_assembled, is_all_polygons_used, is_not_date, is_optimize_emprise, is_optimize_emprise_nodata, no_data_value, size_erode, path_time_log, separ_name="_", pos_date=1, nb_char_date=8, separ_date="", epsg=2154, format_vector='ESRI Shapefile', extension_raster=".tif", extension_vector=".shp", save_results_intermediate=False, overwrite=True):
    ...
    if debug >= 3:
        ...
        print(cyan + "createEmprise() : " + endC + "input_dir : " + str(input_dir))
        print(cyan + "createEmprise() : " + endC + "output_file : " + str(output_file))
   ...
   # Constantes
    ...
    if check and not overwrite:
        print(bold + yellow +  "createEmprise() : " + endC + "Le fichier vecteur d'emprise %s existe déjà : pas d'actualisation" % (output_file) + endC)
        ...
        if not is_not_assembled :
            if is_optimize_emprise or is_optimize_emprise_nodata or is_all_polygons_used :
                ...
                if is_all_polygons_used and is_not_date :
                    fusionNeighbourPolygonsBySameValue(file_vector, output_file, column_name, format_vector)
                    #dissolveVector(file_vector, output_file, column_name, format_vector)
                else :
                    if not geometries2multigeometries(file_vector, output_file, column_name, format_vector) :
...
def getDataToFiels(repertory, is_not_date, is_optimize_emprise, separ_name, pos_date, nb_char_date, separ_date, points_list, ref_dossier_list, name_rep_list, date_list, heure_list):
    ...
    if not is_not_date :
        if debug >= 3:
            print(input_rep_name.split(separ_name))
        try :
            ...
            print(cyan + "createEmprise : " + bold + yellow + "WARNING Référence dossier : " + ref_dossier + " Les parametres de saisies de la date ne sont pas correcetes !!! " + endC)

    if (not is_optimize_emprise) and (ref_dossier in ref_dossier_list) :
         print(cyan + "createEmprise : " + bold + yellow + "WARNING Référence dossier : " + ref_dossier + " Existe déjà dans la liste des dossiers!!! " + endC)
...
def main(gui=False):
    ...
    parser.add_argument('-dir','--input_dir', default="", help="Input directory containing the images directories.", type=str, required=True)
    parser.add_argument('-outf','--output_file', default="",help="Output file containing all polygones.", type=str, required=True)
    ...
    if debug >= 3:
        ...
        print(cyan + "CreateEmprises : " + endC + "input_dir : " + str(input_dir) + endC)
        print(cyan + "CreateEmprises : " + endC + "output_file : " + str(output_file) + endC)
    ...
    # Fonction générale
    createEmprise(input_dir, output_file, not_assembled, all_polygons_used, not_date, optimize_emprise, optimize_emprise_nodata, no_data_value, size_erode, path_time_log, separname, posdate, nbchardate, separdate, epsg, format_vector, extension_raster, extension_vector, save_results_intermediate, overwrite)
