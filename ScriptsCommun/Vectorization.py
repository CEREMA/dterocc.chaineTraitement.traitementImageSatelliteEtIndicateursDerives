...
import os,sys,glob,argparse,gdal,ogr
...
from Lib_vector import relabelVectorFromMajorityPixelsRaster,fusionNeighbourPolygonsBySameValue, cutVectorAll, multigeometries2geometries, cleanLabelPolygons, renameFieldsVector
...
from Lib_postgis import openConnection, closeConnection, dropDatabase, createDatabase, importVectorByOgr2ogr, exportVectorByOgr2ogr, topologyCorrections
...
def vectorizeClassification(image_input, vector_output, name_column, umc_list, tilesize, enable_reaffectation_raster, enable_meanshift_filtering, enable_segmentation, enable_small_region_merging, enable_vectorization, enable_boundaries, boundaries_vector, enable_dissolve, enable_reaffectation_vector, enable_cor_bord, wrongval_list, path_time_log, expression="(im1b1==11000?400:(im1b1==12200?100:(im1b1==21000?300:(im1b1==22000?200:im1b1))))", format_vector='ESRI Shapefile',  extension_raster=".tif", extension_vector=".shp", save_results_intermediate=False, overwrite=True) :
    ...
    for umc in umc_list :
        ...
        uml_label = str(int(round(umc * area_pixel)))
        ...
        # ETAPE 5 : DECOUPAGE DU VECTEUR AU REGARD DE L'EMPRISE POUR GERER CERTAINS BUGS ISSUS DE LA FUSION - A METTRE EN OBLIGATOIRE EN DECOUPANT AU REGARD DE L'EMPRISE DU RASTER A EXTRAIRE AUTOMATIQUEMENT
        ...
        if enable_dissolve : # Si cette gestion des polygones adjacents de même valeur pose problème, utiliser la brique 21 de la chaine (rasterisation puis nouvelle vectorisation)
            ...
            fusionNeighbourPolygonsBySameValue(vector_segmented_merged_cuted_mpoly, vector_segmented_merged_cuted_dissolved, name_column, format_vector)
...
#    Eléments modifiés auccun
#
def vectorizeGrassClassification(image_input, vector_output, name_column, umc_list, enable_reaffectation_raster, enable_vectorization, enable_boundaries, boundaries_vector, enable_dissolve, path_time_log, expression="(im1b1==11000?400:(im1b1==12200?100:(im1b1==21000?300:(im1b1==22000?200:im1b1))))", format_vector='ESRI Shapefile',  extension_raster=".tif", extension_vector=".shp", save_results_intermediate=False, overwrite=True) :
    ...
    epsg = getProjectionImage(image_input)                         # Recuperation de la valeur de la projection du rasteur d'entrée
    ...
    for umc in umc_list :
        ...
        uml_label = round(umc * area_pixel)
        ...
        if enable_vectorization :
            ...
            vectorisationGrass(image_raster, vector_vectorised, area_pixel, uml_label, uml_label, True, format_vector, overwrite)
        ...
        # ETAPE 2 : DECOUPAGE DU VECTEUR AU REGARD DE L'EMPRISE - A METTRE EN OBLIGATOIRE EN DECOUPANT AU REGARD DE L'EMPRISE DU RASTER A EXTRAIRE AUTOMATIQUEMENT
        if enable_boundaries :
        ...
        if enable_dissolve :
            ...
            # Fusion des polygones voisins
            fusionNeighbourPolygonsBySameValue(vector_vectorised_cuted_mpoly, vector_vectorised_cuted_dissolved, name_column, format_vector)
...
def topologicalCorrection(vector_output, epsg, project_encoding, server_postgis, port_number, user_postgis, password_postgis, database_postgis, schema_postgis, path_time_log, format_vector, save_results_intermediate=False, overwrite=True) :
    ...
    topologyCorrections(connection, table_correct_name, geometry_column='geom')
...
def main(gui=False):
    ...
    if correction_sql :
        topologicalCorrection(vector_output, epsg, project_encoding, server_postgis, port_number, user_postgis, password_postgis, database_postgis, schema_postgis, path_time_log, format_vector, save_results_intermediate, overwrite)
