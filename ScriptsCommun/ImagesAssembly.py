...
def selectAssembyImagesByHold(emprise_vector, input_repertories_list, output_file, is_not_assembled, is_zone_date, epsg, is_vrtfile, is_band_stack, is_clean_zero, is_clean_zero_output, clean_zero_value, pixel_size_x, pixel_size_y, no_data_value, separ_name, pos_date, nb_char_date, separ_date, path_time_log, file_out_suffix_error="_error", file_out_suffix_merge="_merge", file_out_suffix_clean="_clean", file_out_suffix_stack="_stack", format_raster='GTiff', format_vector='ESRI Shapefile', extension_raster=".tif", extension_vector=".shp", save_results_intermediate=False, overwrite=True):
    ...
    empr_xmin,empr_xmax,empr_ymin,empr_ymax = getEmpriseVector(emprise_vector, format_vector)
    ...
    if is_not_assembled :
        ...
        for acquisition_date in table_images_list:
            ...
            assemblyImages(images_list, emprise_shape_date, image_output, is_zone_date, pixel_size_x, pixel_size_y, file_out_suffix_merge, file_out_suffix_clean, no_data_value, epsg, EXT_TEXT, format_raster, format_vector, extension_raster, extension_vector, save_results_intermediate, overwrite)
...
###########################################################################################################################################
def assemblyImages(images_list, empr_file, output_file, is_zone_date, pixel_size_x, pixel_size_y, file_out_suffix_merge, file_out_suffix_clean, no_data_value, epsg, ext_txt, format_raster='GTiff', format_vector='ESRI Shapefile', extension_raster=".tif", extension_vector=".shp", save_results_intermediate=False, overwrite=True):
    """
    # ROLE:
    ...
    repertory_cut_temp = repertory_output + os.sep + file_name + SUFFIX_CUT + SUFFIX_TEMP
    repertory_raw_temp_list = []
    if not os.path.isdir(repertory_cut_temp):
        os.makedirs(repertory_cut_temp)

    for imagefile in images_list:
        print(imagefile)
        appendTextFileCR(list_file_tmp, imagefile)
        file_name_tmp = os.path.splitext(os.path.basename(imagefile))[0]
        repertory_raw_temp = repertory_output + os.sep + file_name_tmp + SUFFIX_RAW + SUFFIX_TEMP
        repertory_raw_temp_list.append(repertory_raw_temp)
        vector_emprise_tmp = repertory_raw_temp + os.sep + file_name_tmp + extension_vector

        # Création des dossiers s'ils n'existent pas déjà
        if not os.path.isdir(repertory_raw_temp):
            os.makedirs(repertory_raw_temp)
        # Copie des fichiers brutes
        shutil.copy(imagefile, repertory_raw_temp)

        # Emprise avec l'application createEmprise
        createEmprise([repertory_raw_temp], vector_emprise_tmp, False, False, True, False, True, no_data_value, 40.5, "", "", 0, 0, "", epsg, format_vector, extension_raster, extension_vector, save_results_intermediate, overwrite)

        # Découpage des images à l'emprise optimisé et sans le bord
        image_tmp_cut = repertory_cut_temp + os.sep + file_name_tmp + SUFFIX_CUT + SUFFIX_TEMP + extension_raster
        list_file_str += " " + image_tmp_cut
        if not cutImageByVector(vector_emprise_tmp, imagefile, image_tmp_cut, pixel_size_x, pixel_size_y, False, no_data_value, epsg, format_raster, format_vector) :
            print(cyan + "assemblyImages : " + bold + red + "!!! Une erreur c'est produite au cours du découpage de l'image assemblée : " + image_tmp_cut + ". Voir message d'erreur." + endC, file=sys.stderr)
     """
    ...
    if not save_results_intermediate :
        for repertory_raw_temp in repertory_raw_temp_list :
            if not os.path.isdir(repertory_raw_temp):
                deleteDir(repertory_raw_temp)
        if not os.path.isdir(repertory_cut_temp):
            deleteDir(repertory_cut_temp)
    """
    ...
    if os.path.exists(merge_file_tmp):
        ...
        if not cutImageByVector(empr_file, merge_file_tmp, output_file, pixel_size_x, pixel_size_y, False, no_data_value, epsg, format_raster, format_vector) :
    ...
    for imagefile in images_list:
        ...
        if (imag_xmin >= round_empr_xmax) or (imag_xmax <= round_empr_xmin) or (imag_ymin >= round_empr_ymax) or (imag_ymax <= round_empr_ymin) :
            ...
            if num_features > 0 :
                ...
                if not cutImageByVector(local_emprise_shape, clean_file_output, output_file, pixel_size_x, pixel_size_y, False, no_data_value, epsg, format_raster, format_vector) :
