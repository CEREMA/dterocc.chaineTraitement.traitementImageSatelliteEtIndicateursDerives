...
def classificationLczSql(input_division, input_hre, input_ocs, output_lcz, id_field='id', epsg=2154, format_vector='ESRI Shapefile', postgis_ip_host='localhost', postgis_num_port=5432, postgis_user_name='postgres', postgis_password='postgres', postgis_database_name='lcz_db', postgis_schema_name='public', postgis_encoding='UTF-8', path_time_log='', save_results_intermediate=False, overwrite=True):
    ...
    query += "UPDATE %s SET %s = '9', %s = 9 WHERE %s = 'urban' AND (bur <= 20) AND (hre <= 9);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
    query += "UPDATE %s SET %s = '6', %s = 6 WHERE %s = 'urban' AND (hre <= 9);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
    query += "UPDATE %s SET %s = '5', %s = 5 WHERE %s = 'urban' AND (hre > 9 AND hre <= 30);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
    query += "UPDATE %s SET %s = '4', %s = 4 WHERE %s = 'urban' AND (hre > 30);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
    query += "UPDATE %s SET %s = '3', %s = 3 WHERE %s = '6' AND (bur > 40) AND ((bur + ror) > 70);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
    query += "UPDATE %s SET %s = '2', %s = 2 WHERE %s = '5' AND (bur > 40) AND ((bur + ror) > 70);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
    query += "UPDATE %s SET %s = '1', %s = 1 WHERE %s = '4' AND (bur > 40) AND ((bur + ror) > 70);\n" % (lcz_table, lcz_field, lcz_int_field, lcz_field)
