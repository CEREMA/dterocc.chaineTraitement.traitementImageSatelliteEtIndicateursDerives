...
def unet(input_size, n_filters, n_classes, kernel_size, mode, upconv = False):
    ...
    if mode == "mono":
        ...
        model.compile(optimizer = adam_v2.Adam(learning_rate = 1e-3), loss = "binary_crossentropy", metrics = metrics)
    else:
        ...
        model.compile(optimizer = adam_v2.Adam(learning_rate = 1e-3), loss = SparseCategoricalCrossentropy(from_logits = True), metrics = metrics)
...
def computeTrain(training_input, image_input, vector_input, model_output, image_output, NN, neural_network_mode, size_grid, debord, augment_training, number_class, path_time_log, save_data, overwrite=True, extension_raster=".tif", extension_vector=".shp", format_raster='GTiff', format_vector='ESRI Shapefile', rand_seed=0, epsg=2154, percent_no_data=10, save=False):
    ...
    if model_output == "" :
        ...
        model_save_dir = model_dir + os.sep + model_file_name + EXTENSION_NEURONAL_NETWORK
    ...
    pourcent = int(((img_size*img_size) * percent_no_data) / 100)
    cpt_img_supprime = 0
...
def main(gui=False):
    ...
    if args.percent_no_data != None :
        ...
        if (percent_no_data < 0):
            raise NameError (cyan + "NeuralNetworkClassification : " + bold + red  + "%i not a correct number!" %(percent_no_data) + endC)
